...
angle = (int(float(sys.argv[1]))*47.1111)-50
...
def updatePos():
    with open('/home/go4t/team5/Main_file/update_position.txt', 'w') as file:
        file.write(f'{xbegin};{ybegin};{(orientation+(-diff/47.1111))%360}')
angle_decer = angle - 1000
timestart = time.time()
lasttime = timestart
while (diff < angle_decer and time.time()-timestart < max_time):
    c.set_motor_speed(min(vitesse, vitesse_consigne), max(-vitesse, -vitesse_consigne))
    vitesse += 2
...
while (diff <= angle and time.time()-timestart < max_time):
    c.set_motor_speed(round(max(vitesse, 3)), round(min(-vitesse, -3)))
    vitesse -= 0.2
    if (time.time() - lasttime > 1):
        updatePos()
        lasttime = time.time()
